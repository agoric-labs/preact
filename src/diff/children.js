import { diff, unmount, applyRef, createDiffData } from './index';
...
export function diffChildren(
	...
	for (i = 0; i < newParentVNode._children.length; i++) {
		...
		childVNode = newParentVNode._children[i] = coerceToVNode(childVNode);

		childVNode._parent = newParentVNode;
		childVNode._depth = newParentVNode._depth + 1;

		oldVNode = findOldVNode(childVNode, oldChildren, i);
...
export function selectOldDom(oldParentVNode, excessDomChildren) {
	let oldDom;
	if (excessDomChildren != null && excessDomChildren.length) {
		oldDom = excessDomChildren[0];
	} else if (
		oldParentVNode &&
		oldParentVNode._children &&
		oldParentVNode._children.length
	) {
		oldDom = getDomSibling(oldParentVNode, 0) || null;
	} else {
		oldDom = null;
	}
	return oldDom;
